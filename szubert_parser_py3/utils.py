...
def print_dict_ordered_by_value(dict):
    ...
    if isinstance(list(dict.values())[0], tuple):
        sorted_items = sorted(list(dict.items()), key=lambda x: x[1][0], reverse=True)
    else:
        sorted_items = sorted(list(dict.items()), key=lambda x: x[1], reverse=True)
...
def print_dict_for_table(dict):
    ...
    for k, v in sorted_items:
        if isinstance(v, float):
            ...
            print(str(k) + "   &   " + str(v) + "\\")
    if isinstance(list(dict.values())[0], int):
...
def get_children(node, graph):
    return [child for parent, child in list(graph.keys()) if parent == node and graph[(parent, child)]]


def get_parents(node, graph):
    return [parent for parent, child in list(graph.keys()) if child == node and graph[(parent, child)]]

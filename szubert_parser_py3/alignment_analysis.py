...
def read_in_isi_alignments_part(file_name, parses):
    ...
    alignments = {k - 100: v for k, v in list(alignments_full.items()) if 99 < k < 200}
    ...
    for line in open(file_name, "r"):
        ...
        if split_line:
            ...
            for node_id in jamr_alignments:
                try:
                    other_id = [k for k,v in list(concept_dict_jamr.items()) if v == node_id][0]
                except IndexError:
                    pass
                renamed_jamr_alignments[other_id] = jamr_alignments[node_id]
            normal_alignments = collections.defaultdict(set)
            for jamr_a, entries in list(renamed_jamr_alignments.items()):
...
def alignment_neatness(manual_alignments, verbose=False):
    ...
    for s_id in manual_alignments:
        ...
        highest_alignment = highest_alignment_type(list(alignment_types.keys()))
...
def coverage(file_name, dict_file):
    ...
    for s, inf in list(manual_alignments.items()):
...
def node_alignment_types(manual_alignments, source='amr'):
    ...
    for s_id in manual_alignments:
        ...
        aligned_amr_paths = list(path_alignments.keys())
        ...
        if source == 'amr':
            ...
            total += len(dep_nodes)
            aligned_deps = flatten_list([[x for x in align_list if is_node(x)] for align_list in list(node_alignments.values())])
            for dep_node in dep_nodes:
                if dep_node == 0:
                    continue
                if (dep_node, ) in aligned_deps:
                    if sum([(dep_node, ) == d for d in aligned_deps]) == 1:
                        one_to_one += 1
                    else:
                        one_to_many += 1
                else:
...
def proportion_of_order_preserving_alignments(manual_alignments):
    ...
    for s_id in manual_alignments:
        ...
        if contains_cycles(amr_graph) or contains_cycles(dep_graph):
            ...
            node_alignments = manual_alignments[s_id]['alignments']['nodes']
            for amr_node, aligned_dep in list(node_alignments.items()):
...
def individually_unaligned(manual_alignments):
    for s_id in manual_alignments:
        ...
        node_alignments = manual_alignments[s_id]['alignments']['nodes']
        path_alignments = manual_alignments[s_id]['alignments']['paths']
        aligned_dep = flatten_list([extract_nodes(x) for x in flatten_list(list(node_alignments.values())) if is_node(x)])
        aligned_dep.extend(flatten_list([extract_nodes(x) for x in flatten_list(list(path_alignments.values())) if is_node(x)]))
...
def unaligned_nodes(manual_alignments):
    for s_id in manual_alignments:
        if 'alignments' in manual_alignments[s_id]:
            ...
            amr_nodes = manual_alignments[s_id]['amr_concepts']
            dep_nodes = manual_alignments[s_id]['lemmas']
            amr_nodes_in_alignments = flatten_list([extract_nodes(p) for p in list(path_alignments.keys())])
            amr_nodes_in_alignments.extend([n[0] for n in list(node_alignments.keys())])
            for amr_node in amr_nodes:
                if amr_node not in amr_nodes_in_alignments and amr_node != 'top':
                    unaligned_amr_nodes.append(amr_node)
            dep_nodes_in_alignments = flatten_list([extract_nodes(x) for x in flatten_list(list(node_alignments.values()))])
            dep_nodes_in_alignments.extend(flatten_list([extract_nodes(x) for x in flatten_list(list(path_alignments.values()))]))
            for dep_node in dep_nodes:
...
def unaligned_edges(manual_alignments):
    for s_id in manual_alignments:
        if 'alignments' in manual_alignments[s_id]:
            ...
            node_alignments = manual_alignments[s_id]['alignments']['nodes']
            path_alignments = manual_alignments[s_id]['alignments']['paths']
            amr_edges_in_alignments = [edge for path in list(path_alignments.keys()) for edge in path]
            dep_paths_in_alignments = [x for x in flatten_list(list(node_alignments.values())) if not is_node(x)]
            dep_paths_in_alignments.extend([x for x in flatten_list(list(path_alignments.values())) if not is_node(x)])
...
def proportion_of_unaligned(manual_alignments, structure_type):
    ...
    sum_unaligned_amr = sum([len(v['unaligned']['amr_{}'.format(structure_type)]) for v in list(manual_alignments.values())])
    sum_unaligned_dep = sum([len(v['unaligned']['dep_{}'.format(structure_type)]) for v in list(manual_alignments.values())])
...
def extra_nodes(manual_alignments):
    ...
    for s_id in manual_alignments:
        ...
        alignment_pairs = list(path_alignments.items())
        alignment_pairs.extend(list(node_alignments.items()))
...
def unexpected_nodes(to_inspect, counterpart, node_alignments, path_alignments, side):
    ...
    if side == 'dep':
        ...
        aligned_to_dep = [a for (a, dep_list) in list(node_alignments.items()) if counterpart in dep_list if not is_node(a)]
        aligned_to_dep.extend([a for (a, dep_list) in list(path_alignments.items()) if counterpart in dep_list])
        expected_structures = flatten_list([[a for d in dep_list if contained_within(d, counterpart)] for (a, dep_list) in list(node_alignments.items())])
        expected_structures.extend(flatten_list([[a for d in dep_list if contained_within(d, counterpart)] for (a, dep_list) in list(path_alignments.items())]))
...
def find_only_lexically_aligned_nodes(manual_alignments):
    for s_id in manual_alignments:
        ...
        dep_node_alignments = flatten_list([list(v) for k,v in list(node_alignments.items())])
        dep_node_alignments.extend(flatten_list([list(v) for k,v in list(structure_alignments.items())]))
...
def non_ontology_entities(sent_dict):
    ...
    for s_id in sent_dict:
        ...
        for (parent, child), relation in list(sent_dict[s_id]['amr_graph'].items()):
            if relation == ':name':
...
def extract_aligned_targets(source_nodes, alignments_nodes):
    ...
    for s in source_nodes:
        for amr, dep_list in list(alignments_nodes.items()):
            if amr[0] == s:
...
def see_proportions_of_unaligned(manual):
    ...
    total_amr_nodes = sum([len(x['amr_concepts']) - 1 for x in list(manual.values())])
    total_dep_nodes = sum([len(x['lemmas']) - 1 for x in list(manual.values())])
    total_amr_edges = sum([len(x['amr_graph']) - 1 for x in list(manual.values())])
    total_dep_edges = sum([len(x['dep_graph']) - 1 for x in list(manual.values())])

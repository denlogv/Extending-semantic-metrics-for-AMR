...
def get_easy_alignment_points(sentences, neg_dict):
    ...
    for s_id in list(sentences.keys()):
...
def add_particle_node_alignments(alignments, amr_concepts, lemmas, amr_relations, dep_relations):
    ...
    for c_id, l_ids in list(node_alignments.items()):
        current_alignments = copy.deepcopy(l_ids)
        for (l_id,) in current_alignments:
            particle_children = [n2 for (n1, n2), e_lab in list(dep_relations.items()) if n1 == l_id and e_lab == ":compound-prt"]
...
def add_entity_type_alignments(alignments, amr_concepts, amr_relations):
    ...
    for (n, ), deps in list(n_alignments.items()):
...
def add_obvious_edge_alignments(alignments, dep_relations):
    ...
    for c_id, l_list in list(align_copy.items()):
        if len(l_list) > 1:
            for l1 in l_list:
                for l2 in l_list:
                    if l1 != l2:
...
def aligned_through_neg_dict(node1, node2, morph_model, neg_dict):
    if node2 in list(neg_dict.keys()):
...
def get_node_alignments_step1(alignments, amr_concepts, lemmas, amr_relations, dep_relations, neg_dict, morph_model):
    ...
    light_verbs = ['take', 'have', 'make', 'did', 'get', 'give', 'carry']
    for l_id in lemmas:
        if lemmas[l_id] in light_verbs and l_id not in flatten_list([dep_list for (amr, dep_list) in list(alignments['nodes'].items())]):
            extended_alignments(l_id, dep_relations, alignments['nodes'])
...
def add_reversed_node_alignment_dict(alignments):
    reversed_a = {'nodes': collections.defaultdict(set),
                  'paths': collections.defaultdict(set)}
    for c_id, l_ids in list(alignments["nodes"].items()):
        for l_id in l_ids:
...
def create_node_pairs(alignments, limited=True):
    node_pairs = []
    aligned_nodes = [n for n in list(alignments['nodes'].keys()) if alignments['nodes'][n]]
...
def get_inferred_alignment_points(easy_alignments):
    ...
    """
    for s_id in list(easy_alignments.keys()):
...
def extend_path(source_p, target_p, source_g, alignments, source_type):
    ...
    if source_type == 'DG':
        ...
        aligned_to_target_n = [[amr for amr, dep_list in list(aligned_n.items()) if (t,) in dep_list] for t in target_n]
...
def check_case(d_path, d_graph):
    ...
    for edge in nmod_edges:
        tail = edge[2]
        new_edges = [(e1, lab, e2) for (e1,e2), lab in list(d_graph.items()) if e1 == tail and lab in [":case", ":mark"]]
    for edge in path:
        tail = edge[2]
        new_edges.extend([(e1, lab, e2) for (e1,e2), lab in list(d_graph.items()) if e1 == tail and lab == ":mwe"])
    if new_edges:
...
def conjunction_expansion(amr_path, amr_graph, dep_graph, amr_concepts, alignments):
    ...
    for amr_path, dep_paths in list(path_aligments.items()):
        changed_amr = False
        path = list(amr_path)
        nodes = extract_nodes(amr_path)
...
def amr_overlaps_in_sentence(p_alignments, node_dict, verbose):
    amr_structures = list(p_alignments.keys())
...
def dep_overlaps_in_sentence(n_alignments, p_alignments, lemmas, verbose):
    dep_structures = [x for x in flatten_list(list(n_alignments.values())) if not is_node(x)]
    dep_structures.extend([x for x in flatten_list(list(p_alignments.values())) if not is_node(x)])
    ...
    for s_id in alignments:
        ...
        for dep_overlap in overlaps[s_id]['dep']:
            for dep in dep_overlap:
                amr_sides_n = [(amr, dep_side) for (amr, dep_side) in list(n_align.items()) if dep in dep_side]
                amr_sides_p = [(amr_p, dep_side) for (amr_p, dep_side) in list(p_align.items()) if dep in dep_side]
...
def percolate(source, node_buffer, graph, nodewise_path_dict, direction, source_id):
    ...
    node_buffer.extend(list(zip([source for _ in range(len(next_nodes))], next_nodes)))
    if node_buffer:
...
def check_for_meeting_points(nodewise_path_dict):
    return [n for n in list(nodewise_path_dict.keys()) if (nodewise_path_dict[n]['from_1_up'] or nodewise_path_dict[n]['from_1_down'])

...
def process_amr(amr_string, jamr=False, isi=False):
    ...
    if jamr and isi:
        ...
        concept_dict = {k: v[0] for k, v in list(concept_dict_combined.items())}
        concept_dict_jamr = {k: v[1][0] for k, v in list(concept_dict_combined.items())}
        concept_dict_isi = {k: v[1] for k, v in list(isi_concept_dict_combined.items())}
    if jamr:
        ...
        concept_dict = {k: v[0] for k, v in list(concept_dict_combined.items())}
        concept_dict_isi = {k: v[1] for k, v in list(concept_dict_combined.items())}
    else:
...
def parse_amr(tokens, concept_dict, relations, current_relation, open_concepts, none_counter, jamr_counter = {},
    ...
    if not tokens:
        ...
        if first == ')':
            open_concepts.pop()
            if jamr or isi:
                cleared_counter = {level: v for level,v in list(jamr_counter.items()) if level <= nesting_level}
                nesting_level -= 1
            else:
...
def draw_graphs(sent_dict, output_dir, save_graphs=False, want_amr=True, want_dep=True, special_subgraphs=None):
    ...
    for s in sent_dict:
        ...
        if len(word_list) > 15:
            title = '\n'.join([' '.join(segment) for segment in [word_list[i:i+15] for i in range(0, len(word_list), 15)]])
        if want_dep:
            dependencies = sent_dict[s]['dep_graph']
            word_dict = sent_dict[s]['words']
            lemmas = [str(l) + '/ ' + word_dict[l] for l in list(word_dict.keys())]
            ...
            if special_subgraphs:
                ...
                special_edge_labels = {tup: rel for (tup, rel) in list(dep_edge_labels.items()) if tup in special_subgraphs[s][3]}
                other_edge_labels = {tup: rel for (tup, rel) in list(dep_edge_labels.items()) if tup not in special_subgraphs[s][3]}
        ...
        if want_amr:
            ...
            amr_concepts = [d + "/ " + amr_concept_dict[d] for d in list(amr_concept_dict.keys())]
            ...
            if special_subgraphs:
                ...
                special_edge_labels = {tup: rel for (tup, rel) in list(amr_edge_labels.items()) if tup in special_subgraphs[s][1]}
                other_edge_labels = {tup: rel for (tup, rel) in list(amr_edge_labels.items()) if tup not in special_subgraphs[s][1]}
...
def write_out_parses(sent_dict, filename):
    ...
    for s_id in sent_dict:
        ...
        id_pairs = sorted(list(dependencies.keys()), key=lambda x: x[1])

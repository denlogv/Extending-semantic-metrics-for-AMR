...
def avg_vector_for_compound(compound, split_by='_'):
    ...
    return sbert_model.encode(words, convert_to_tensor=True), num_nodes
...
def maybe_sim(a, b, cutoff=0.5, diffsense=0.5, simfun=cosine_sim, mwp ="split"):
    ...
    if "_" in a:
        parts = a.split('_')
        if parts[-1]:
            a_vec, num_nodes = avg_vector_for_compound(a)
            found_a_vec = True
    if "_" in b:
        parts = b.split('_')
        if parts[-1]:
            b_vec, num_nodes_b = avg_vector_for_compound(b)
    ...
    if not (found_a_vec or found_b_vec):
        a_vec, b_vec = sbert_model.encode([a, b], convert_to_tensor=True)
    elif found_a_vec:
        b_vec = sbert_model.encode(b, convert_to_tensor=True)
    elif found_b_vec:
        a_vec = sbert_model.encode(a, convert_to_tensor=True)
        ...
        for j in range(0, len(instance2)):
            ...
            if cur_instance_triple1[0].lower() == cur_instance_triple2[0].lower():
                ...
                if node1 in meta[1]['alignments_dict'] and node2 in meta[2]['alignments_dict']:
                    ...
                    similarity_aligned = sbert_sim(sents_embs, sents_encoded, toks1_span, toks2_span)
...
def sbert_sim(sents_embs, sents_encoded, phrase1, phrase2):
    ...
    phrase1_embs_indices = x_in_y(sbert_model.tokenize(phrase1), sents_encoded[0])
    phrase2_embs_indices = x_in_y(sbert_model.tokenize(phrase2), sents_encoded[1]) 
    ...
    return cosine_sim(torch.mean(sents_embs_new[0, phrase1_embs_indices], 0), 
...
def precompute_embeddings(amr_file1, amr_file2):
    ...
    sents_embs = sbert_model.encode(sents_encoded, convert_to_tensor=False, 
                                    convert_to_numpy=False,
                                    output_value='token_embeddings',
                                    is_pretokenized=True,
                                    show_progress_bar=False)
    
    sents_embs = torch.nn.utils.rnn.pad_sequence(sents_embs, batch_first=True)
    ...
    for i in range(int(num_amrs)):
        ...
        if -1 in best_mapping and meta_available:            
            ...
            for amr_a_nodes, amr_b_node in updated_best_mapping.items():                
                ...
                if len(amr_a_nodes) > 1 and amr_a_nodes_text and amr_b_nodes_text != 'Null':

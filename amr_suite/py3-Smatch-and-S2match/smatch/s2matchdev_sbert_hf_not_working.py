...
def build_arg_parser():
    ...
    parser.add_argument('-sbert', required=False, type=str, default='sentence-transformers/bert-base-nli-mean-tokens',
                        help='Name of the SBert-Model to find in the HuggingFace repo')
...
def cosine_sim(a, b):
    #cosine similarity
    return torch.nn.CosineSimilarity(0)(a, b)
...
def get_best_match(instance1, attribute1, relation1,
                   ...
                   prefix1, prefix2, sents_embs, sents_encoded, 
                   meta, cutoff, diffsense, simfun, mwp):
    """
    ...
    (candidate_mappings, weight_dict) = compute_pool(instance1, attribute1, relation1,
                                                     instance2, attribute2, relation2,
                                                     prefix1, prefix2, sents_embs, 
                                                     sents_encoded, meta, cutoff, 
                                                     diffsense, simfun, mwp)
...
def maybe_sim(a, b, cutoff=0.5, diffsense=0.5, simfun=cosine_sim, mwp ="split"):
    ...
    if a_wo_sense and b_wo_sense:
        a, b = a_wo_sense, b_wo_sense
    elif a_wo_sense:
        a = a_wo_sense
    elif b_wo_sense:
        b = b_wo_sense
    ...
    with torch.no_grad():
        words_encoded = sbert_tokenizer([a, b], padding=True, truncation=True, max_length=128, return_tensors='pt')
        words_embs = sbert_model(**words_encoded)[0]        
        a_vec, b_vec = mean_pooling(words_embs, words_encoded['attention_mask'])
...
def maybe_has_sim(a, b, sim_dict, cutoff=0.5, diffsense=0.5, 
                  simfun=cosine_sim, mwp ="split"):
...
def compute_pool(instance1, attribute1, relation1,
                 instance2, attribute2, relation2,
                 prefix1, prefix2, sents_embs, sents_encoded, meta, 
                 cutoff=0.5, diffsense=0.5, simfun=cosine_sim, mwp="split"):
    """
        ...
        for j in range(0, len(instance2)):
            ...
            if cur_instance_triple1[0].lower() == cur_instance_triple2[0].lower():
                ...
                if node1 in meta[1]['alignments_dict'] and node2 in meta[2]['alignments_dict']:
                    ...
                    toks2_indices = full_span([meta[2]['alignments_dict'][node2]])
                    
                    toks1_span = ' '.join([meta[1]['toks'][i] for i in toks1_indices])
                    toks2_span = ' '.join([meta[2]['toks'][i] for i in toks2_indices])
                    
                    try:
                        similarity = sbert_sim(sents_embs, sents_encoded, toks1_span, toks2_span)
                    except RuntimeError:
                        pass
                        #print(toks1_span, toks2_span)
                        #print(sents_encoded)
                
                else:
                    value_1 = instance1[i][2].lower()
                    value_2 = instance2[j][2].lower()
                    similarity = maybe_has_sim(value_1, value_2, sim_dict, cutoff=cutoff, 
        ...
        for j in range(i+1, len(mapping)):
            ...
            """    
                ...
                if new_match_num != cur_match_num + sw_gain:
                    print(new_match_num, cur_match_num, sw_gain
                    print(match, new_match
...
def mean_pooling(sents_embs, attention_mask):
    
    input_mask_expanded = attention_mask.unsqueeze(-1).expand(sents_embs.size()).float()
    sum_embeddings = torch.sum(sents_embs * input_mask_expanded, 1)
    sum_mask = torch.clamp(input_mask_expanded.sum(1), min=1e-9)
    return sum_embeddings / sum_mask


def sbert_sim(sents_embs, encoded_ids, phrase1, phrase2):
    sentence_embeddings = mean_pooling(sents_embs, get_new_attention_mask(phrase1, phrase2, encoded_ids, sbert_tokenizer))
    
    return cosine_sim(sentence_embeddings[0], sentence_embeddings[1]).item()
...
def precompute_embeddings(amr_file1, amr_file2):
    ...
    sents_encoded = sbert_tokenizer(amrs_sents, padding=True, truncation=True, 
                                    max_length=128, return_tensors='pt')
    with torch.no_grad():
        sents_embs = sbert_model(**sents_encoded)[0]
        
    sents_embs = mean_pooling(sents_embs, sents_encoded['attention_mask'])

    return sents_embs, sents_encoded
...
def main(arguments):
    ...
    sents_embs_all, sents_encoded_all = precompute_embeddings(args.f[0].name, args.f[1].name)
    print('Finished! Running S2Match...', file=sys.stderr)
    
    print(f'Embeddings dims     = {sents_embs_all.shape}', file=sys.stderr)
    print(f'Sents encoding dims = {sents_encoded_all["input_ids"].shape}', file=sys.stderr)
    print(f'Attention mask dims = {sents_encoded_all["attention_mask"].shape}', file=sys.stderr)
    ...
    while True:
        ...
        if cur_amr1 == "" and cur_amr2 == "":
            break
        if cur_amr1 == "":
            print("Error: File 1 has less AMRs than file 2", file=sys.stderr)
            print("Ignoring remaining AMRs", file=sys.stderr)
            break
        if cur_amr2 == "":
            print("Error: File 2 has less AMRs than file 1", file=sys.stderr)
            print("Ignoring remaining AMRs", file=sys.stderr)
            break
        
        
        sents_encoded = {'input_ids':sents_encoded_all['input_ids'][i:i+2,],
                         'attention_mask':sents_encoded_all['attention_mask'][i:i+2,]}
        sents_embs = sents_embs_all[i:i+2,]
        
        print(sents_encoded['attention_mask'].shape)
        ...
        if -1 in best_mapping and meta_available:            
            ...
            for amr_a_nodes, amr_b_node in updated_best_mapping.items():                
                ...
                amr_b_nodes_text = amr2text_b_current[amr_b_node] if amr_b_node != 'Null' else 'Null'
                ...
                if len(amr_a_nodes) > 1 and amr_a_nodes_text and amr_b_nodes_text:
                    mapping_updated = True
 
                    sim = sbert_sim(sents_embs, sents_encoded, amr_a_nodes_text, amr_b_nodes_text)
                    
                    subtract_from_score = weight_dict[(nodes2indices_a[amr_a_nodes[0]], 
                                                       nodes2indices_b[amr_b_node[0]])][-1]
                    add_to_score = sim*len(amr_a_nodes) - subtract_from_score
...
if __name__ == "__main__":
    ...
    sbert_tokenizer = AutoTokenizer.from_pretrained(args.sbert)
    sbert_model = AutoModel.from_pretrained(args.sbert)
        
    main(args)
    
# python smatch/s2matchdev_sbert.py -f smatch/testa smatch/testb -diffsense 0.9 -cutoff 0.2 -v --ms
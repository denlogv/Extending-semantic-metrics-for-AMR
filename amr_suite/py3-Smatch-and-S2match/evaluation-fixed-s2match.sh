...
# Evaluation script. Run as: ./evaluation.sh <parsed_data> <gold_data>
out=`python3 smatch/s2match.py --pr -f "$1" "$2" -vectors $VECS -similarityfunction $SF -cutoff $TAU -diffsense $DIFFSENSE`
out=($out)
echo 'S2match -> P: '${out[1]}', R: '${out[3]}', F: '${out[6]} | sed 's/.$//'

python3 unlabel.py "$1"  > 1.tmp
python3 unlabel.py "$2"  > 2.tmp
out=`python3 smatch/s2match.py --pr -f 1.tmp 2.tmp -vectors $VECS -similarityfunction $SF -cutoff $TAU -diffsense $DIFFSENSE`
out=($out)
...
python3 scores-enhanced-s2align.py "1.tmp" "2.tmp" $VECS $SF $TAU $DIFFSENSE
